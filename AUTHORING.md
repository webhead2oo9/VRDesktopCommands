...
with a page dropdown when the command has pages.
...
```json
{
    "format": 2,
    "name": "wifisetup",
    "description": "How to set up Virtual Desktop over WiFi",
    "accent_color": 5793266,
    "thumbnail_url": "https://cdn.example.com/wifi-icon.png",
    "blocks": [ ... ],
    "pages": [
        { "name": "router", "title": "Router Settings", "description": "Recommended router config", "blocks": [ ... ] }
    ]
}
```
...
| `pages`         | *        | Extra views behind a dropdown. Max 25 (the in-guild editor caps at 22). Each page: unique `name` (same pattern as command names), optional `title`/`description` (≤100 chars, shown in the dropdown), optional `accent_color` (defaults to the command's), optional `thumbnail_url` (per view — pages do **not** inherit the command's), and its own `blocks`. |

\* At least one of `blocks` / `pages` is required. Prefer having top-level `blocks` — without
them the initial slash reply is empty until the user picks a page.
...
{ "type": "set_property", "target": { "kind": "page", "page": "wiring" }, "property": "thumbnail_url", "old": null, "new": "https://cdn.example.com/opal-icon.png" }
